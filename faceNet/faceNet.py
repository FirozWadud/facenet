...
class FaceNet:
    ...
    def load_anchors(self, faces_path: str):
        ...
        for person_dir in stow.ls(faces_path):
            person_name = stow.basename(person_dir)
            person_images = []
            for face_path in stow.ls(person_dir.path):
                face_image = cv2.imread(face_path.path)
                normalized_encode = self.encode(face_image)
                person_images.append(normalized_encode)
            anchors[person_name] = np.mean(person_images, axis=0)
    ...
    def __call__(self, frame: np.ndarray) -> np.ndarray:
        ...
        face_crops = {index: {"name": "Unknown", "tlbr": tlbr} for index, tlbr in
                      enumerate(self.detector(frame, return_tlbr=True))}
        for key, value in face_crops.items():
            ...
            if np.max(distances) > self.threshold:
                person_name = list(self.anchors.keys())[np.argmax(distances)]
                face_crops[key]["name"] = person_name
